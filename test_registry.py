...
async def test_registry():
    ...
    immediate_candidates = registry.list_candidates(
        role="BackupAgent", required_traits={"strategy": "immediate_after_index"}
    )
    ...
    no_network_candidates = registry.list_candidates(
        role="BackupAgent", policy_context={"network_allowed": False}
    )
    ...
    print("\n✓ Test 9: quarantine")
    registry.quarantine(
        "BackupAgent_batch_v2", "Repeated policy violations", society_memory=society_memory
    )
    ...
    # Test 10: Retire
    print("\n✓ Test 10: retire")
    registry.retire("BackupAgent_batch_v2", "Underperforming", society_memory=society_memory)
    agent = registry.get_agent("BackupAgent_batch_v2")
