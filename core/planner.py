...
class Planner:
    ...
    async def receive_message(self, message: Message) -> Optional[Message]:
        ...
        if not filtered:
            ...
            return await self.escalate_to_user(
                "No agents available due to policy constraints",
                message
            )
        ...
        if not ranked:
            ...
            return await self.escalate_to_user(
                "Could not determine best agent for this task",
                message
            )
    ...
    async def rank_candidates(
        ...
        if recommended_id:
            ...
            recommended = next(
                (c for c in candidates if c.agent_id == recommended_id),
                None
            )
            if recommended:
    ...
    async def route_to_best(
        ...
        task = await self.create_task(
            ...
            metadata={"intent": intent.task_type}
        )
    ...
    async def handle_escalation(self, message: Message):
        ...
        if "missing capability" in message.content.lower():
            ...
            await self.escalate_to_user(
                f"Policy block: {message.content}",
                message
            )
        else:
            # Generic error - notify Curator
            await self.escalate_to_curator(
                f"Task error: {message.content}",
                message
            )
