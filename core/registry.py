...
class Registry:
    ...
    def list_candidates(
        ...
        for agent_info in self.agents.values():
            ...
            if required_traits:
                if not all(
                    agent_info.traits.get(key) == value
                    for key, value in required_traits.items()
                ):
                    continue
    ...
    def _check_policy_compatibility(
        ...
        if sensitivity == "high":
            ...
            if agent.safety_profile.needs_network and agent.safety_profile.irreversible_ops_possible:
                return False
